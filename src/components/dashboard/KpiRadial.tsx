...
const COLORS = { blue: '#3b82f6', amber: '#f59e0b', pink: '#ec4899', green: '#10b981' }

export default function KpiRadial({ label, percent = 0, color = 'pink', value, delta, description, loading }: Props) {
  ...
  return (
    <div className="rounded-2xl border bg-white p-6 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="mt-2 flex items-center justify-between">
        ...
        <div className="text-sm text-gray-500">{loading ? '…' : `${pct.toFixed(0)}%`}</div>
      </div>
      <div className="mt-1 flex items-baseline gap-2">
        <div className="text-xl font-bold text-gray-900">{loading ? '—' : value ?? '—'}</div>
        <div className={`text-xs font-medium ${d > 0 ? 'text-emerald-600' : d < 0 ? 'text-red-600' : 'text-gray-500'}`}>
          {loading ? '' : deltaText}
        </div>
      </div>
      {description && (
